...
import { addUserApi, deleteUserById, editUserApi, getAllUsers, getSalesUsersApi, getUserById } from '../../../../services/user.service'
...
export const getSalesUsers = createAsyncThunk('appUsers/getSalesUsers', async (user, { dispatch, getState }) => {

  try {
    const { data: res } = await getSalesUsersApi(user)
    if (res.data) {
      toastSuccess(res.data.message)
    }
